...
server <- function(input, output) {
  observeEvent(input$roll, {
    song_params <- list(
      Key = paste(sample(keys, 1), sample(modes, 1)),
      Tempo = weighted_sample(tempo_ranges, tempo_weights),
      Time_Signature = weighted_sample(time_signatures, time_weights),
      Chord_Progression = weighted_sample(progressions, progression_weights),
      Mood = sample(moods, 1)
    )
    output$output <- renderText({
      paste(names(song_params), ":", unlist(song_params), collapse = "\n")
    })
  })
}
